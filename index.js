...
(function () {
  ...
  */
  ...
  /* Yiyi added this extra line for fixing the highlight for single file on GitHub */
  .hljs.js-file-line {
...
`;
  ...
  // register the K language to highlight.js
  hljs.registerLanguage("k", k);

  // Your code here...
  const codeBlocks = document.querySelectorAll("pre[lang=k]");
  for (let i = 0; i < codeBlocks.length; i++) {
    const codeBlock = codeBlocks[i];
    const highlight = hljs.highlight("k", codeBlock.innerText, true).value;
    codeBlock.classList.add("hljs");
    codeBlock.innerHTML = `<code>${highlight}</code>`;
  }

  if (window.location.href.endsWith(".k")) {
    const lines = document.querySelectorAll("td.js-file-line");
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (line.innerText && line.innerText.trim().length) {
        const highlight = hljs.highlight("k", line.innerText, true).value;
        line.classList.add("hljs");
        line.innerHTML = `<code>${highlight}</code>`;
      }
    }
  }
})();
